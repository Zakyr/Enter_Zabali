	...
	while(state==1){
		...
		switch(titleScreen(1)){
			case '1':
				hPotions=0;
				sPotions=0;
				gotoxy(38,24);printf("Loading Game"); Sleep(1000);
				...
				break;
			case '2':
				gotoxy(38,24);printf("Loading Scores"); Sleep(1000);
				highScores();
				break;
			case '3':
				gotoxy(38,24);printf("Loading"); Sleep(1000);
				guide();
				system("pause");
				break;
			case '4':
				UL();
				gotoxy(38,24);printf("Exiting"); Sleep(1000);
				system("cls");
				return 0;
			//default:
...
void inventory(int p){
	...
	if(p==0){
		...
		gotoxy(29,22);printf("[1] Health Potions: %2d/10",hPotions);
		gotoxy(29,23);printf("[2] Stamina Potions: %2d/10",sPotions);
...
int titleScreen(int p){
	...
	if(p==1){
		...
		gotoxy(37,21);printf("[2] HIGH SCORES");
...
void guide(){
	...
	gotoxy(5,10);printf("-> [Goblin] & [Giant Spider] gives +.5 to MAX HEALTH");
	gotoxy(5,12);printf("-> [Beholder] & [Orc] gives +.5 to ATTACK");
	gotoxy(5,14);printf("-> [Skeleton], [Wererat] & [Vampire] gives +.5 to SPEED");
...
void comment(int p){
	...
	if(p==1){ ////enemy attacking////
		...
		printf("[dodged] SUCCESSFUL"); Sleep(1000); gotoxy(7,15);
		printf("                   "); 	
	}
	else if(p==11){
