...
void instructions(){
	gotoxy(30,8);printf("-> use WASD to move");
	gotoxy(30,9);printf("-> %c are walls", wall);
	gotoxy(30,10);printf("-> %c is the door to the next level", door);
}

void stats(){
	int i;
	gotoxy(30,4); printf("Health:\t");
		color(2);
		for(i=0; i<(h/2/2); i++){
			printf("%c", 254);
		}
