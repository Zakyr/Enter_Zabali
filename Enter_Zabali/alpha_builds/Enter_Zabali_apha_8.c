...
void inventory(int p){
	...
	if(p==0){
		...
		gotoxy(29,22);printf("[1] Health Potions: %d/5",hPotions);
		gotoxy(29,23);printf("[2] Stamina Potions: %d/5",sPotions);
...
void comment(int p){
	...
	if(p==1){ ////enemy attacking////
		...
		printf("Inventory full. Drinking potion."); Sleep(1000); gotoxy(7,15);
		printf("                                ");	
	}
	else if(p==6){
...
void endScreen(){
	...
	gotoxy(26,18);printf(" \\______/      \\__/    |______||_|   \\_\\");
		...
		gotoxy(30,20);printf("        -> Score: %d <-",score);
}
